...
const getMediasFromTaxonomies = (taxonomies: Taxonomy[]): { id: string; singular_name: string }[] => {
    return taxonomies
        .filter((taxonomy) => taxonomy.seed?.includes("medio"))
        .map((taxonomy) => ({ id: taxonomy.id, singular_name: taxonomy.singular_name }));
};
const getTopicsFromTaxonomies = (taxonomies: Taxonomy[]): { id: string; singular_name: string }[] => {
    return taxonomies
...
export const mapCards = (gridCards: UiGridCard[]): Featured[] => {
    return gridCards?.flatMap((gridCard) => {
        return gridCard.cards?.map((card) => {
