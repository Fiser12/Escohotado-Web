...
export const getAuthorFromTaxonomies = (taxonomies: Taxonomy[]): Taxonomy | undefined =>
  taxonomies?.find(
    (taxonomy) => taxonomy.breadcrumbs?.some((b) => b.url?.includes('autor')) == true,
  )
...
export const getMediasFromTaxonomies = (taxonomies: Taxonomy[]): CategoryModel[] => {
  return taxonomies
    .filter((taxonomy) => taxonomy.breadcrumbs?.some((b) => b.url?.includes('source')) == true)
    .map(mapTaxonomyToCategoryModel)
}

export const getTopicsFromTaxonomies = (taxonomies: Taxonomy[]): CategoryModel[] => {
  return taxonomies
    .filter((taxonomy) => taxonomy.breadcrumbs?.some((b) => b.url?.includes('tema')) == true)
    .map(mapTaxonomyToCategoryModel)
}

export const getSelectableTaxonomies = (taxonomies: Taxonomy[]): CategoryModel[] => {
  return taxonomies
