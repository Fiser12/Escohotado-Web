...
import { Block, Config, RichTextAdapterProvider } from 'payload'
...
export function lexicalEditorExcluding(blocksSlugs: BlockSlug[]): Config['editor'] {
  const blocks = () => defaultBlocks().filter((block) => !blocksSlugs.includes(block.slug as BlockSlug))
  return buildLexical(blocks)
}
